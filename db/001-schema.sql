...
CREATE TABLE config (
  ...
  -- Verifica a autenticidade do webhook de pagamento (HMAC no Mercado Pago, token no Asaas).
  webhook_pagamento TEXT NULL,
...
CREATE TABLE node_audit_log (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  empresa INT NOT NULL,
  -- NULL = ação do sistema, sem usuário por trás (ex.: pagamento confirmado por webhook do
  -- provedor). A origem fica em `detalhes`.
  usuario INT NULL,
